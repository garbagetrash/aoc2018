...
type Input = (HashMap<(usize, usize), TrackType>, Vec<Cart>);
...
pub fn part1(input: &Input) -> String {
    ...
    let sleep_time = 0;
    let (track_map, mut carts) = input.clone();
        ...
        if active_cart_cnt == 1 {
            for cart in &carts {
                if cart.orientation != 'X' {
                    ...
                    return format!("{},{}", cart.pos.0, cart.pos.1);
                }
            }
        }
...
mod test {
    ...
    fn test_part1example1() {
        let input = load_input(&read_to_string("inputs/13example1.txt").unwrap());
        assert_eq!(part1(&input), "0,3");
    }

    #[test]
    fn test_part1example2() {
        let input = load_input(&read_to_string("inputs/13example2.txt").unwrap());
        assert_eq!(part1(&input), "7,3");
    }

    #[test]
    fn test_part2() {
        let input = load_input(&read_to_string("inputs/13example3.txt").unwrap());
